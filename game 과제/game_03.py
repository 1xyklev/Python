...
def start_game():
    ...
    while True:
        if not ball.hit_bottom:
            # 추가 기능: 공 일시정지 기능
            # ball.paused == True이면 ball.draw() 실행 안 함
            if not ball.paused:
                ball.draw()
...
class Ball:
    def __init__(self, canvas, color, paddle):
        ...
        # 추가 기능: 공 일시정지 상태 변수
        # space 키를 누르면 True/False로 토글됨
        self.paused = False
        self.canvas.bind_all('<KeyPress-space>', self.toggle_pause)

    # 추가 함수: 일시정지 기능 토글
    # Space 키를 누르면 self.paused가 반전됨
    def toggle_pause(self, evt):
